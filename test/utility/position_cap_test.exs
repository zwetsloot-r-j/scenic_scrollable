defmodule Scenic.Scrollable.PositionCapTest do
  ...
  test "init", %{default: default} do
    ...
    assert PositionCap.init(%{min: {:horizontal, 0}}) == %PositionCap{min: {:some, {:horizontal, 0}}}
    assert PositionCap.init(%{max: {:vertical, 10}, min: {:vertical, 0}}) == %PositionCap{max: {:some, {:vertical, 10}}, min: {:some, {:vertical, 0}}}
  end

  test "cap", %{default: default} do
    assert PositionCap.cap(default, {-999999, 999999}) == {-999999, 999999}

    PositionCap.init(%{min: {0, -10}, max: {10, 20}})
    |> PositionCap.cap({-999999, 999999})
    |> Kernel.==({0, 20})
    |> assert
